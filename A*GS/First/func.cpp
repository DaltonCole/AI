...
// Note: Tried to use a priority queue, was unsuccessful
	...
	for(int i = (goals.size() - 1); i >= 0; i--)
	{
		...
		do
		{
			...
			if(go_left(eval, frontier, explored, goals))
			{
				...
				temp = (new point((eval -> m_x - 1), eval -> m_y, eval -> color, dist, eval, 'l'));
				dist = find_d(temp, goals[i]);
				// If previous node came from left as well, favor this node
				if(temp -> previous_direction == eval -> previous_direction && dist >= 2)
				{
					dist--;
				}
				temp -> distance = dist;
			...
			if(go_right(eval, frontier, explored, max_x, goals))
			{
				...
				temp = (new point((eval -> m_x + 1), eval -> m_y, eval -> color, dist, eval, 'r'));
				dist = find_d(temp, goals[i]);
				// If previous node came from right as well, favor this node
				if(temp -> previous_direction == eval -> previous_direction && dist >= 2)
				{
					dist--;
				}
			...
			if(go_up(eval, frontier, explored, goals))
			{
				...
				temp = (new point((eval -> m_x), (eval -> m_y - 1), eval -> color, dist, eval, 'u'));
				dist = find_d(temp, goals[i]);
				// If privous node came from the north as well, favor this node
				if(temp -> previous_direction == eval -> previous_direction && dist >= 2)
				{
					dist--;
				}
			...
			if(go_down(eval, frontier, explored, max_y, goals))
			{
				...
				temp = (new point((eval -> m_x), (eval -> m_y + 1), eval -> color, dist, eval, 'd'));
				dist = find_d(temp, goals[i]);
				// If previous node came from the south as well, favor this node
				if(temp -> previous_direction == eval -> previous_direction && dist >= 2)
				{
					dist--;
				}
